...
\rare{-{}-disable-multi-pass}{Make only one attempt to parse C code, rather
  than trying to passskip some tokens or expand macros.}
...
\rare{-{}-test-spacing}{Like -{}-ctestall, but ensures that the spacing is
  the same as in the .res file. Tests whose name ends in "\_spacingfailure"
  are known to fail and will not affect the total score. Tests whose name ends
  in "\_failure" will be skipped.}
...
\rare{-{}-no-gotos}{Cause gotos to simply fall through to the next
  line. This may make a semantic patch/match run faster, but in general does
  not respect the semantics of the source code.}
...
\developer{-{}-cocci-internals}{When using -{}-parse-cocci, use a different
pretty printer that reveals Coccinelle's internal types.}

\developer{-{}-c-internals}{When printing an object in a Python script,
use a different pretty printer that reveals Coccinelle's internal types.}
...
\minimum{-{}-help, -{}-longhelp}{ A complete listing of the available
commands.}
