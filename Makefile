...
install-metainfo:
	if test "x$(METAINFO_DIR)" != "xno"; then \
		...
		$(INSTALL_DATA) extra/io.github.coccinelle.coccinelle.metainfo.xml \
			$(DESTDIR)$(METAINFO_DIR)/io.github.coccinelle.coccinelle.metainfo.xml; \
	fi
...
uninstall-metainfo :
	rm -f $(DESTDIR)$(METAINFO_DIR)/io.github.coccinelle.coccinelle.metainfo.xml
	rmdir --ignore-fail-on-non-empty -p \
